...
export default function Hero({ profile }: { profile: Profile }) {
    ...
    return (
        <section className="relative isolate overflow-hidden">
            ...
            <div className="relative mx-auto grid max-w-6xl items-end gap-6 px-5 pb-0 pt-8 sm:gap-8 sm:pt-10 md:min-h-[min(760px,calc(100dvh-var(--header-height))] md:grid-cols-12 md:gap-0 md:px-8 md:pt-14">
                {/* Portrait — first on mobile for visual anchor */}
                <motion.div
                    initial={reduce ? false : { opacity: 0, y: 16 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.65, delay: 0.05, ease: [0.22, 1, 0.36, 1] }}
                    className="relative z-10 order-1 flex items-end justify-center md:order-2 md:absolute md:inset-y-0 md:end-0 md:w-[min(54%,560px)] md:justify-end lg:w-[min(50%,540px)]"
                >
                    {profile.photo_path && (
                        <div className="portrait-stage relative w-full max-w-[14rem] sm:max-w-[17rem] md:max-w-none md:w-full md:translate-x-6 lg:translate-x-10 rtl:md:-translate-x-6 rtl:lg:-translate-x-10">
                ...
                {/* Copy */}
                <div className="relative z-20 order-2 flex flex-col justify-center pb-8 sm:pb-10 md:order-1 md:col-span-7 md:pb-16 lg:col-span-7">
                    <motion.div
